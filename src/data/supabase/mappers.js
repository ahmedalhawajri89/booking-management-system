...
/** @param {Row} row @returns {import('@/types').Booking} */
export function toBooking(row) {
  return {
    ...
    history: (row.booking_events ?? [])
      .map((e) => ({ at: e.at, type: e.type, summary: e.summary }))
...
export function fromBooking(b, orgId) {
  return {
    id: b.id,
    org_id: orgId,
...
export function fromCustomer(c, orgId) {
  return {
    id: c.id,
    org_id: orgId,
    name: c.name,
...
export function toCatalog(services, resources, hours, links) {
  ...
  return {
    ...
    resources: resources.map((r) => ({ id: r.id, name: r.name, isActive: r.is_active })),
