...
import type { Booking, BusinessHours, Service } from '@/types'
 ...
 */
function localDayWithWeekday(weekday: number): Date {
...
describe('overlaps', () => {
  ...
  it('detects full containment in both directions', () => {
    const outer = [d('2030-03-03T09:00:00Z'), d('2030-03-03T12:00:00Z')] as const
    const inner = [d('2030-03-03T10:00:00Z'), d('2030-03-03T10:30:00Z')] as const
...
describe('hasConflict', () => {
  ...
  it.each(['cancelled', 'no_show'] as const)('lets a %s booking release its time', (status) => {
    const freed = booking({ status })
    const candidate = booking({ id: 'b2' })
    expect(hasConflict(candidate, [freed])).toBe(false)
  })

  it.each(['pending', 'confirmed'] as const)('treats a %s booking as blocking', (status) => {
...
describe('generateSlots', () => {
  ...
  it('never offers a slot that would run past closing', () => {
    ...
    expect(new Date(last.endAt).getTime()).toBeLessThanOrEqual(
      new Date(at(OPEN_DAY, 12)).getTime(),
    )
  })
  ...
  it('reserves the buffer, not just the service duration', () => {
    ...
    const first = slots[0]!
    const minutes =
      (new Date(first.endAt).getTime() - new Date(first.startAt).getTime()) / 60000
