...
/**
 * @typedef {object} ScrollScenes
 * @property {import('vue').Ref<number>} progress      0–1 across the whole pinned container.
 * @property {import('vue').Ref<number>} sceneIndex    Which scene the viewer is on.
 * @property {import('vue').Ref<number>} sceneProgress 0–1 inside the current scene — this is what drives the artifact.
 * @property {(i: number) => void} goToScene
 ...
 * @param {import('vue').Ref<number> | (() => number)} sceneCount
 * @returns {ScrollScenes}
 */
export function useScrollScenes(container, sceneCount) {
  const reduced =
